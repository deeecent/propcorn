...
task("deploy:propcornccip", "Deploy Propcornccip Contract").setAction(
  async function (_, { ethers }) {
    ...
    const propcornccip = await ccipPropcornFactory
      .connect(deployer)
      .deploy("CHANGETHIS", deployer.address); //fill in the Router address from https://docs.chain.link/ccip/supported-networks
